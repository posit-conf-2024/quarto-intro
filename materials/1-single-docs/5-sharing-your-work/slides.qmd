...
Custom Formats (aka Templates) are a way to bundle fonts, images, logos, filters, formatting, and metadata for use by others.

. . .

*Install a custom format*

- `quarto add <githubname>/<reponame>` to install the format
- `quarto use template <githubname>/<reponame>` to install the format with a sample qmd file.

. . .

*Use the custom format*

Set `format: <extentionname>-<baseformat>`. Example:
...
## Our Turn: Custom Revealjs {.our-turn}

1. In the same directory as your slide deck, run at the terminal:

   ```bash
   quarto add qmdr/qmdrslides
   ```
   
2. Change your document yaml to use `format: qmdrslides-revealjs`
...
## A Website in 5 min {.our-turn}

Use the RStudio IDE to...

1. Create New Project > New Directory > Quarto Website
2. Build > Render Website (shift-Cmd-B)
3. From the terminal:
   `quarto publish`
